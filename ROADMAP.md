...
- [x] **试点交付包（Pilot Delivery）**：把标杆 POC 从「骨架」升级为「可交付资产」。
  ...
  - **校准铺包契约**：`provision.py` / `common.py` 端点与载荷对齐平台真实路由——KB 创建 `/api/kb`、文档灌入 `/api/kb/{id}/ingest`（multipart）、技能 `/api/skills`、剧本 `/api/agent/playbooks`（双段写法 `/api/kb/kb`、`/api/skills/skills` 经后续真实服务栈铺包实测被证为 405 并修正为单段，见 §29）；apply 时注入租户/项目隔离键，并对剧本 `defaults` 做嵌套 logical_id→real_id 递归替换。
  ...
  - **校验**：`validate_poc.py` 两套 0 error；`provision.py --dry-run` 端点/载荷正确；回滚 dry-run 反向顺序正确；agent-service 新文件语法编译通过、迁移模块导入无误。`0004_playbook` 与既有 `0001_initial` 同样使用 PostgreSQL JSONB（SQLite 无法渲染 JSONB，故 alembic 仅支持 PG——属项目级既有约束，非本次引入）。详见 VERIFICATION_REPORT 第 25 节；**`0004_playbook` 真实 PG 建表已闭环验证（PostgreSQL 16.14 上 `agent_playbooks` 表 + `defaults`/`scenario_flow` JSONB 列 + 租户索引均就位，见 §25.5）**；**端到端铺包实测亦已完成**——`provision.py --apply`（10 步全 OK）真实写入 `agent_playbooks`、含嵌套 logical_id 替换与租户注入，`--rollback`（6/6 OK，反向顺序正确）真实删除，全链路在真实 HTTP 层跑通，并修复脚本两处真实 bug（state 资源 kind 错配、mock 网关 204 响应），见 §28。
  - **真实本地服务栈端到端铺包实测（§29）**：在本地自有端口（非智衍 EvolvIQ、非生产）真实拉起全部 8 个微服务（gateway 8000 / model-gateway 8001 / auth 8002 / skills 8003 / mcp 8004 / kb 8005 / audit 8006 / agent 8007），用 `provision.py --apply --gateway-url http://localhost:8000` 跑真实建包 + 回滚，全程 admin JWT + 租户隔离键 `e2e00000-0000-0000-0000-000000000013`（KB/agent 用 `tenant_id` 字符串、skills/mcp 用 `project_id` UUID）。实测暴露并修复 5 类真实 bug：① skills/mcp/kb 三服务入口缺 `shared` 自注入 `ModuleNotFoundError`（仿网关加 `sys.path.insert`）；② 陈旧 `knowledge.db` 缺 `tenant_id` 列致 KB 创建 500（删库后 `create_all` 重建 schema）；③ provision/common 端点与真实路由不符（`/api/kb/kb`→`/api/kb`、`/api/skills/skills`→`/api/skills`、文档灌入 `/api/kb/{id}/ingest`）；④ skills/mcp 缺 DELETE 路由（回滚漏删，已补 `DELETE /skills/{skill_id}`、`DELETE /mcp/servers/{server_id}` 含级联清理）；⑤ httpx 默认 `trust_env=True` 读 Windows 注册表系统代理把 `127.0.0.1` 也送代理致整批 502（两处请求加 `trust_env=False` 修复）。**实测结果：apply 9/10**（KB 创建 + 文档灌入 3 + 技能 3 + MCP 创建 1 + 剧本 1 全 201/202；仅 `manufacturing` 的 MES 同步因本机无真实 MES 外部依赖 502，符合预期）、**rollback 6/6**（剧本→MCP→技能×3→KB 全 200，新增删除路由生效），栈复测 `GET /api/kb` 返回空、资源全清。详见 §29。
