...
- [x] **生产数据层 · PostgreSQL**：所有服务 `DATABASE_URL` 已配置化；agent-service 已真实接入 PostgreSQL 16.14（Docker 容器 `wb-pg`）端到端跑通——ORM 自动建 `agent_runs`（id/project_id/user_id/prompt/answer/steps_json/model/status/created_at）、`steps_json` 在 PG 落 JSONB、HTTP 4/4 + DB 8/8 二次校验全绿（详见 `VERIFICATION_REPORT.md` 第 12 节）。其他服务（auth/audit/skills/mcp）仍用 sqlite，下阶段统一迁 PG
